...
from sub.string_hasher import hash_string
Flag=0
# flag is used to know where the input number should go 

api='1953235531:AAH7QQWpdoYusowOUnialeTrUa12n3b'
...
def Truecaller(update: Update,context) -> None:
	update.message.reply_text('Enter a number without +91 or share a contact')
	global Flag
	Flag = 1

# /smsbomber calls this funtion
def Smsbomber(update: Update,context) -> None:
	update.message.reply_text('Enter a number to spam or share a contact')
	global Flag
	Flag = 2
...
def Number(update: Update,context:CallbackContext) -> None:
	...
	if Flag == 1:
		...
		hashed_mobile_number = hash_string(mobile_number)
		number_exists = db.check_before_bombing(hashed_mobile_number)
...
def Contact(update,context: CallbackContext):
	...
	if Flag == 1:
		...
		hashed_mobile_number = hash_string(phoneNumber)
		number_exists = db.check_before_bombing(hashed_mobile_number)
...
def protect(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        hashed_mobile_number = hash_string(mobileNo)
        exists = db.check_if_exists(chat_id,hashed_mobile_number)

        if mobileNo and exists:
            update.message.reply_text(f'{mobileNo} in protected list')

        elif mobileNo and not exists:
            db.protect_number(chat_id,hashed_mobile_number)
...
def unprotect(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        hashed_mobile_number = hash_string(mobileNo)
        if mobileNo:
            count = db.unprotect_number(chat_id,hashed_mobile_number)
            if count:
